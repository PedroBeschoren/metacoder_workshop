...
Here we will load  data from 3 species in the Brassicacea family experiment: Arabidopsis thaliana, Brassica oleraceae and Isatis tinctoria. This data has been heavily filtered to reduce complexity and size. These plant species have been exposed to two hormone treatment stresses: Methyl Jasmonate to mimic herbivory and Salicilic acid to mimic pathogens. We sequenced the rhizosphere microbial communities days after stress induction.
...
otu_table(ps_rarefied)[1:5,1:5] # 5 otu table first rows across 5 first columns
tax_table(ps_rarefied)[1:5,] # 5 taxonomy first rows across all columns
ps_rarefied@sam_data [1:5,1:5]# metadata, 5 first rows across 5 first columns


``` 

Both phyloseq and metacoder can take any feature table with taxonomic ranks and metadata. Therefore you can include community data about fungi, insects, plants etc. To import any kind of data into phyloseq, you can check this tutorial: https://joey711.github.io/phyloseq/import-data.html#phyloseq-ize_data_already_in_r
...
#remove detailed taxonomic info ("ASV_id") by updating the tax table with a subset of the tax table. if we include the ASVs the tree will look too busy
...
The tree might be too dense/complex to evaluate directly, so let's remove taxa names that can be  unfamiliar like  "phylum NB1-J"
