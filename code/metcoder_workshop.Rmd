...
ps_rarefied@sam_data [1:10]# metadata
...
## 3.1 - check taxmap object you just created
```{r}
# the taxmap object is a data container, just like phyloseq
taxmap_obj

# this object has much more than what you can see on one print, just like much other R output you generate. explroe it with "$" or "@" and tab-completion

taxmap_obj$data$otu_table # otu tables...
taxmap_obj$data # more of the data itself
...
heat_tree(.input = taxmap_obj, # your taxmap object from the metacoder package
          ...
          output_file = "./results/heat_tree_test.pdf") # The layout algorithm that initializes node locations
...
lets now compare ASV abudnaces between two groups of samples, such as plant species in brassicaceae lineage I and II. 
...
taxmap_obj$data$tax_occ_2lineages <- calc_n_samples(obj = taxmap_obj, 
                                                    data = "tax_abund", 
                                                    cols = taxmap_obj$data$sample_data$sample_id, # What columns of sample data to use
...
# set differental log ratio to 0 based on p values
...
If you have more than 2 groups, like we have 3 different stresses, you can plot a matrix of heat trees with a dedicated function from the same package. before we do that, let's first calculate taxon abundance, prevalence, and log-fold changes like we did in chuncks 3.5.1 to 3.5.3
...
These trees are iteresting, but as you start exploring real data and multiple options you will need a more efficienc way of running all this code. for this, we create a custom function that handles all of it at once, 

### 3.7.1 - Define a custom function

this custom function will: 

1 - truncate the taxonomy table to genues level,  
2 - remove the g__ and f__ etc from taxonomy table
3 - parse the phyloseq object into a taxmap object, 
4 - calculate taxa occurence, abudance and log-fold differences between groups of samples
5 - plot a heat tree
...
```{r}
# run custom function
heat_tree_stress<-phyloseq_to_heat_tree_matrix2(ps_object = ps_rarefied_filt, sample_group = "Stress")
heat_tree_lineage<-phyloseq_to_heat_tree_matrix2(ps_object = ps_rarefied_filt, sample_group = "Sp_Lineage_Walden")
...
new_heat_tree1<-phyloseq_to_heat_tree_matrix2(ps_object = ps_rarefied_filt, sample_group = "")
...
Our trees so far stopped at genus level, because ploting 2000 ASVs would overcrowd the plot. *Show the "Stress" effect at ASV level, but only on Burkholderiales. What do you have to change, and where?*

```{r}
# change the function to include ASV_id, and only visualize taxa within Burkholderiales.
phyloseq_to_heat_tree_matrix_2<-function(ps_object, sample_group){
  # this function output is a heat trees comparing metadata. the input is based on a phyloseq object
  # ps object =  a phyloseq object, containing sample metadata, otu table, and taxonomy table
  # sample_group = the name of a column in your emtadata that you want to compare in the heat tree. it has to be quoted.
  # this function will create a matrix of heat trees if your metadata has more than 2 groups. it should fail if it has only 1 group
...
burkholderiales_stress<-phyloseq_to_heat_tree_matrix_2(ps_object = ps_rarefied_filt, sample_group = "Stress")

```

#### 3.7.3.2 - remove kingdom from taxonomy table

Our trees so far have k__Bacteria as a shared root. *What will happen if you remove that taxonomic information? to make this change, What do you have to alter?*

```{r}
# change the function to remove k__Bacteria
phyloseq_to_heat_tree_matrix_3<-function(ps_object, sample_group){
  # this function output is a heat trees comparing metadata. the input is based on a phyloseq object
  # ps object =  a phyloseq object, containing sample metadata, otu table, and taxonomy table
  # sample_group = the name of a column in your emtadata that you want to compare in the heat tree. it has to be quoted.
  # this function will create a matrix of heat trees if your metadata has more than 2 groups. it should fail if it has only 1 group
...
#remove unecessary taxonomic info (dada2id, "s__" and "ASV_id") by updating the tax table with a subset of the tax table
tax_table(ps_object)<-tax_table(ps_object)[,2:6]
...
noKingdom_stress<-phyloseq_to_heat_tree_matrix_3(ps_object = ps_rarefied_filt, sample_group = "Sp_full_name")
...
What if we want to visualize other types of taxa-associated data, like p values from deseq2, importance in random forest predictions, or degree in a network? You can do that, as long as the vector of values you use as node_size or node_colour have the same lenght as the number of taxon names you are plotting. You can for example add another data layer to the taxmap object, or provide data from an independent df as teh argument of node_size or node_colour.

lets first check some of the taxmap object structure, then create some random data and add it to the heat tree
...
#let's see what si actually used as input for node size and color
n_obs(taxmap_obj)
...
#make up some random data new_data2
set.seed(1)
...
```{r}
# run the custom function over a list of phyloseq objects
